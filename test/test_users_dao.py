...
class TestUsersDAO:
    ...
    def temp_db_url(self):
        ...
        # Use a temporary database URL for testing
        test_db_name = f"test_auth_{os.getpid()}"
        test_url = f"postgresql://postgres:postgres@localhost:5432/{test_db_name}"

        yield test_url

    @pytest.fixture
    def dao_with_db(self, temp_db_url):
        """Create a UsersDAO instance with a temporary database."""
        with patch("src.database.connection.DATABASE_URL", temp_db_url):
            yield UsersDAO()
    ...
    def test_dao_context_manager(self, dao_with_db):
        ...
        with dao_with_db as dao:
            ...
            assert isinstance(dao.conn, psycopg.Connection)
            assert isinstance(dao.cur, psycopg.Cursor)
    ...
    def test_dao_context_manager_cleanup(self, dao_with_db):
        ...
        except (psycopg.OperationalError, AttributeError):
    ...
    def test_execute_query_with_params(self, dao_with_db):
        ...
        with dao_with_db as dao:
            ...
            dao._execute_query(
                "INSERT INTO users(login, password_hash, created_at) VALUES (%s, %s, NOW())",
                (login, password_hash),
            )
